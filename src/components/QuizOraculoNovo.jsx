...
export default function QuizOraculoNovo({ config }) {
  ...
  return <div className="quiz-crystal fixed inset-0 z-50 flex flex-col items-center overflow-y-auto px-4 py-6 sm:py-8"><Aurora /><main className="quiz-layout relative z-10 my-auto flex w-full max-w-lg flex-col py-4">
    ...
    {etapa === 'inicio' && <section className="quiz-xquiz-start text-center"><div className="quiz-xquiz-brand"><span className="quiz-xquiz-brand__spark">✦</span>{config.productName || 'Safira Oráculo'}</div><div className="quiz-xquiz-constellation" aria-hidden><span className="quiz-xquiz-constellation__light" /><img className="quiz-xquiz-object quiz-xquiz-object--moon" src="/quiz-assets/3d/crescent-star.webp" alt="" width="64" height="64" loading="lazy" decoding="async" /><img className="quiz-xquiz-object quiz-xquiz-object--heart" src="/quiz-assets/3d/purple-heart.webp" alt="" width="64" height="64" loading="lazy" decoding="async" /><img className="quiz-xquiz-object quiz-xquiz-object--main" src="/quiz-assets/3d/crystal-ball.webp" alt="" width="128" height="128" loading="lazy" decoding="async" /></div><p className="quiz-xquiz-kicker">CONSULTA GUIADA</p><h1 className="quiz-titulo">Vamos olhar com mais clareza para o seu momento?</h1><p className="quiz-apoio">Responda algumas perguntas e, ao final, escolha as cartas da sua consulta.</p><button className="quiz-xquiz-button" onClick={() => setEtapa('pergunta')}>INICIAR MINHA CONSULTA <span aria-hidden>→</span></button><p className="quiz-xquiz-footnote">Online, particular e no seu ritmo</p></section>}
    {etapa === 'pergunta' && <section className="quiz-painel quiz-xquiz-panel"><p className="quiz-xquiz-counter">Pergunta {pergunta + 1} de {TELAS.length}</p><h2 className="quiz-titulo text-center">{tela.pergunta}</h2><div className="mt-5 flex flex-col gap-3">{tela.opcoes.map((opcao, i) => <button key={opcao} className="quiz-opcao quiz-xquiz-option" onClick={() => responder(i)}><span className="quiz-opcao__ic quiz-xquiz-3d"><img src={tela.imagens[i]} alt="" width="40" height="40" loading="lazy" decoding="async" /></span><span className="flex-1">{opcao}</span><span className="quiz-opcao__seta">›</span></button>)}</div></section>}
    {etapa === 'preparo' && <section className="quiz-painel quiz-xquiz-panel text-center"><div className="quiz-xquiz-symbol mx-auto"><img src="/quiz-assets/3d/crystal-ball.webp" alt="" width="128" height="128" loading="lazy" decoding="async" /></div><p className="quiz-apoio">Obrigada por compartilhar um pouco do seu momento.</p><h2 className="quiz-titulo">Escolha as cartas que mais chamarem a sua atenção.</h2><button className="quiz-xquiz-button" onClick={() => setEtapa('cartas')}>ESCOLHER MINHAS CARTAS</button></section>}
    {etapa === 'cartas' && <section className="quiz-xquiz-cards"><h2 className="quiz-titulo text-center">Escolha as cartas da sua consulta</h2><p className="quiz-apoio text-center">Escolha 3 cartas, uma de cada vez.</p><p className="quiz-xquiz-counter text-center">{escolhidas.length} / {totalCartas} cartas escolhidas</p><div className="quiz-card-choice-grid quiz-xquiz-card-grid">{cartasDoBaralho.map((carta, index) => { const ordem = escolhidas.indexOf(index); const rev = ordem >= 0 ? reveladas[ordem] : null; return <button key={`${carta.nome}-${index}`} className={`quiz-carta ${ordem >= 0 ? 'quiz-carta--virada' : ''}`} disabled={ordem >= 0 || escolhidas.length >= totalCartas} onClick={() => escolherCarta(index)}><span className="quiz-carta__inner"><span className="quiz-carta__face quiz-carta__back">{carta.verso ? <img src={carta.verso} alt="Carta fechada" width="160" height="240" loading="lazy" decoding="async" /> : <CartaVerso simbolo={carta.simbolo} />}</span><span className="quiz-carta__face quiz-carta__front">{rev?.frente && <img src={rev.frente} alt={rev.nome} width="160" height="240" loading="lazy" decoding="async" />}{ordem >= 0 && <span className="quiz-carta__num">{ordem + 1}</span>}</span></span></button>; })}</div></section>}
    {etapa === 'carta-final' && <section className="quiz-painel quiz-xquiz-panel text-center"><h2 className="quiz-titulo">Escolha a sua carta:</h2><div className="mt-5 grid grid-cols-3 gap-3">{reveladas.slice(0, 3).map((carta, i) => <button key={carta.nome} onClick={() => setCartaFinal(i)} className={`quiz-xquiz-final-card ${cartaFinal === i ? 'is-selected' : ''}`}><img src={carta.frente} alt={carta.nome} width="160" height="240" loading="lazy" decoding="async" /></button>)}</div><button className="quiz-xquiz-button mt-6" onClick={continuar}>CONTINUAR PARA MINHA LEITURA</button></section>}
    {etapa === 'analise' && <section className="quiz-painel quiz-xquiz-panel quiz-xquiz-analysis text-center"><div className="quiz-xquiz-analysis__orb"><img src="/quiz-assets/3d/crystal-ball.webp" alt="" width="144" height="144" loading="lazy" decoding="async" /></div><p className="quiz-xquiz-label">ANÁLISE EM ANDAMENTO</p><h2 className="quiz-titulo">Estamos preparando sua leitura</h2><p className="quiz-apoio">Cruzando suas respostas e as cartas que você escolheu.</p></section>}
